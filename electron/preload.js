  ...
  for (const path of candidates) {
    if (fs.existsSync(path)) {
      try {
        ...
        try {
          appendLogLinePre(
            "[preload] readLockfile content=" + content.replace(/\r?\n/g, " ")
          );
        } catch (e) {}
...
const electronAPI = {
  ...
  onDraftStarting: (callback) => ipcRenderer.on("draft-starting", callback),
  ...
  onAcceptanceProgress: (callback) =>
    ipcRenderer.on("acceptance-progress", callback),
  ...
  onSpectatorUnmuted: (callback) =>
    ipcRenderer.on("spectator-unmuted", callback),
  ...
  getBackendApiUrl: () => {
    ...
    const HARDCODED_BACKEND_URL = "http://192.168.1.8:8080/";

    // If BACKEND_URL is provided in the environment, normalize it and return
    const raw =
      process.env && process.env["BACKEND_URL"]
...
electronAPI.identifyPlayer = (payload) => {
  try {
    return ipcRenderer.invoke("lcu:identify", payload);
  } catch (e) {
...
electronAPI.gameActionAcceptMatch = (matchData) => {
  try {
    return ipcRenderer.invoke("game-action-accept-match", matchData);
  } catch (e) {
...
electronAPI.gameActionDeclineMatch = (matchData) => {
  try {
    return ipcRenderer.invoke("game-action-decline-match", matchData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionCancelMatch = (matchData) => {
  try {
    return ipcRenderer.invoke("game-action-cancel-match", matchData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionJoinQueue = (queueData) => {
  try {
    return ipcRenderer.invoke("game-action-join-queue", queueData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionLeaveQueue = (queueData) => {
  try {
    return ipcRenderer.invoke("game-action-leave-queue", queueData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionPickChampion = (pickData) => {
  try {
    return ipcRenderer.invoke("game-action-pick-champion", pickData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionBanChampion = (banData) => {
  try {
    return ipcRenderer.invoke("game-action-ban-champion", banData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionSelectLane = (laneData) => {
  try {
    return ipcRenderer.invoke("game-action-select-lane", laneData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionConfirmDraft = (draftData) => {
  try {
    return ipcRenderer.invoke("game-action-confirm-draft", draftData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionVoteWinner = (voteData) => {
  try {
    return ipcRenderer.invoke("game-action-vote-winner", voteData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionReportResult = (resultData) => {
  try {
    return ipcRenderer.invoke("game-action-report-result", resultData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionSurrender = (surrenderData) => {
  try {
    return ipcRenderer.invoke("game-action-surrender", surrenderData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionMuteSpectator = (muteData) => {
  try {
    return ipcRenderer.invoke("game-action-mute-spectator", muteData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionUnmuteSpectator = (unmuteData) => {
  try {
    return ipcRenderer.invoke("game-action-unmute-spectator", unmuteData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionPing = (pingData) => {
  try {
    return ipcRenderer.invoke("game-action-ping", pingData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.gameActionHeartbeat = (heartbeatData) => {
  try {
    return ipcRenderer.invoke("game-action-heartbeat", heartbeatData);
  } catch (e) {
    return Promise.resolve(false);
  }
};
...
electronAPI.getLCULockfileInfo = () => {
  ...
  if (!info) return null;
  const overrideHost =
    process.env && process.env["LCU_HOST"]
      ? String(process.env["LCU_HOST"])
      : "127.0.0.1";
  return {
    host: overrideHost,
    port: info.port,
...
electronAPI.lcu.getCurrentSummoner = async () => {
  const result = await electronAPI.lcu.request(
...
electronAPI.lcu.getCurrentRanked = async () => {
  ...
  if (result && result.queues && Array.isArray(result.queues)) {
    ...
    if (solo) {
      ...
      console.log(
        `[preload] currentMMR calculado: ${mmr} (${solo.tier} ${solo.division} ${solo.leaguePoints}LP)`
      );
    }
  }
...
electronAPI.lcu.getGameflowPhase = async () =>
  electronAPI.lcu.request("/lol-gameflow/v1/gameflow-phase", "GET");
electronAPI.lcu.getSession = async () =>
  electronAPI.lcu.request("/lol-gameflow/v1/session", "GET");
electronAPI.lcu.getMatchHistory = async () => {
  try {
    console.log("[preload] getMatchHistory: fetching summoner...");
    const summoner = await electronAPI.lcu.request(
      "/lol-summoner/v1/current-summoner",
      "GET"
    );
    ...
    console.log(
      "[preload] getMatchHistory: fetching matches for summonerId",
      summonerId
    );
    ...
    console.log(
      "[preload] getMatchHistory: result type =",
      typeof result,
      "length =",
      result?.length
    );
...
try {
  if (
    ...
    console.log(
      "[preload] ipcRenderer.on disponível:",
      typeof electronAPI.ipcRenderer?.on
    );
  } else {
...
(async function autoConfigure() {
  try {
    ...
    const url = backend.endsWith("/api")
      ? `${backend}/lcu/configure`
      : `${backend}/api/lcu/configure`;
    const explicitHost =
      process.env && process.env["LCU_HOST"]
        ? String(process.env["LCU_HOST"])
        : null;
    const candidateHosts = explicitHost
      ? [explicitHost]
    ...
    for (const h of candidateHosts) {
      ...
      try {
        ...
        console.log(
          "[preload] autoConfigure attempt failed for host=",
          h,
          String(e)
        );
      }
